...
An MPC is an alternative and more advanced method of process control (e.g. See the [PID Control](https://github.com/marcomarasca/SDCND-PID-Controller) project for a simple version of process control) that is used to control a process while satisfying a set of constraints. In particular the problem is reframed in a way to reduce it to an optimization problem. In our case the solution to the optimization problem is the "ideal" trajectory that the vehicle should follow.
...
The application uses **[Ipopt and CppAD](https://projects.coin-or.org/Ipopt)** to find a solution for the cost function, please refer to [this document](https://github.com/marcomarasca/SDCND-Model-Predictive-Control/blob/master/install_Ipopt_CppAD.md) for installation instructions.
...
6. ```git clone https://github.com/marcomarasca/SDCND-MPC```
